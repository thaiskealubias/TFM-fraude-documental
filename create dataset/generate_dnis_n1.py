...
def get_validity_date(birth_date_str, force_caducado=False):
    """
    ...
    Si force_caducado=True, devuelve una fecha anterior al día actual.
    """
    if force_caducado:
        # Forzar fecha caducada (entre 2020 y 2024)
        año_caducidad = random.randint(2020, 2024)
        mes_caducidad = random.randint(1, 12)
        dia_caducidad = random.randint(1, 28)
        return f"{dia_caducidad:02d}/{mes_caducidad:02d}/{año_caducidad}"
...
def create_dni_image(full_name, nif, birth_date_str, filename, force_caducado=False):
    """
    Crea una imagen de DNI realista
    Si force_caducado=True, la fecha de validez será anterior al día actual.
    """
    ...
    validity_date = get_validity_date(birth_date_str, force_caducado)
